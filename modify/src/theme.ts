...
Modified by Sportis JSC (2026) — palette sync integration
...
import { applySportisPalette, getSportisTheme, setSportisTheme, mapSportisThemeToElement, mapElementThemeToSportis } from "./sportis-palette-bridge";

export const DEFAULT_THEME = "light";

// Guard: prevent outbound cookie sync until inbound sync completes
let _sportisThemeSyncReady = false;
...
export async function setTheme(theme?: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        const switchTheme = function (): void {
            ...
            // Apply Sportis palette bridge after theme stylesheet is loaded
            const isDarkTheme = stylesheetName.includes("dark");
            applySportisPalette(isDarkTheme);

            // Outbound sync: write theme to cookie only after inbound sync is done
            if (_sportisThemeSyncReady) {
                const useSystemTheme = SettingsStore.getValue("use_system_theme");
                setSportisTheme(mapElementThemeToSportis(useSystemTheme, stylesheetName));
            }
...
/**
 * Sync Sportis theme preference into Element's SettingsStore on app init.
 * Reads from shared cookie/localStorage and applies if different from current.
 * Call this AFTER ThemeWatcher is initialized — it updates SettingsStore then
 * triggers setTheme() to apply the visual change.
 */
export async function syncSportisThemeOnInit(): Promise<void> {
    try {
        const sportisTheme = getSportisTheme();
        if (!sportisTheme) {
            // No Sportis preference — export current Element theme to cookie
            const useSystem = SettingsStore.getValue("use_system_theme");
            const current = SettingsStore.getValue("theme") || "light";
            setSportisTheme(mapElementThemeToSportis(useSystem, current));
            // Still apply palette (user may have set palette without changing theme)
            applySportisPalette(current.includes("dark"));
            logger.info("Sportis theme: no preference found, exported current Element theme");
            return;
        }

        const { useSystem, theme } = mapSportisThemeToElement(sportisTheme);
        const currentUseSystem = SettingsStore.getValue("use_system_theme");
        const currentTheme = SettingsStore.getValue("theme");
        let changed = false;

        if (currentUseSystem !== useSystem) {
            await SettingsStore.setValue("use_system_theme", null, SettingLevel.DEVICE, useSystem);
            logger.info(`Sportis theme: set use_system_theme=${useSystem}`);
            changed = true;
        }
        if (!useSystem && currentTheme !== theme) {
            await SettingsStore.setValue("theme", null, SettingLevel.DEVICE, theme);
            logger.info(`Sportis theme: set theme="${theme}"`);
            changed = true;
        }

        // Force visual theme change after SettingsStore update
        if (changed) {
            await setTheme(useSystem ? undefined : theme);
            logger.info(`Sportis theme: applied "${sportisTheme}"`);
        } else {
            // Theme unchanged but palette may differ — apply palette directly
            const currentTheme = SettingsStore.getValue("theme") || "light";
            applySportisPalette(currentTheme.includes("dark"));
        }
    } finally {
        // Enable outbound sync only after inbound sync is complete
        _sportisThemeSyncReady = true;
    }
}
